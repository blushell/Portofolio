...
const windowStyle = computed(() => ({
  transform: isMaximized.value ? 'none' : `translate(${position.value.x}px, ${position.value.y}px)`,
  zIndex: zIndex.value,
  minWidth: '300px',
  minHeight: '200px',
...
const toggleMaximize = () => {
  if (isMaximized.value) {
    position.value = constrainPosition(previousPosition.value.x, previousPosition.value.y)
...
<template>
  <div
    ref="windowRef"
    class="bg-dracula-current rounded-lg shadow-xl overflow-hidden"
  ...
  >
    <!-- Window Title Bar -->
    <div 
      class="bg-dracula-background px-4 py-2 cursor-move flex items-center justify-between select-none"
      @mousedown="startDrag"
    ...
    <!-- Image Container -->
    <div class="relative overflow-auto p-4 bg-dracula-background h-[calc(100%-2.5rem)]">
      ...
      >
        ...
        <div class="text-dracula-comment text-sm break-all">{{ imagePath }}</div>
      </div>
    </div>
  </div>
</template>
